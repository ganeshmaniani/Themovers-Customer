...
class DisposalSearchLocation extends ConsumerStatefulWidget {
  final String label;

  const DisposalSearchLocation({super.key, required this.label});

  @override
  ConsumerState<DisposalSearchLocation> createState() =>
      _DisposalSearchLocationConsumerState();
}

class _DisposalSearchLocationConsumerState
    extends ConsumerState<DisposalSearchLocation> {
  bool isLoading = false;

  List<DisposalPredictions> predictionsList = [];
  List<DisposalGeocodeList> geocodeList = [];
  ...
  void placesAutoComplete(String query) async {
    ...
    ref
        ...
        .autoCompletePlaceList(
          DisposalPlaceEntities(query: query, key: AppKeys.placeApiKey),
        )
        .then((res) => res.fold(
            (l) => {
                  predictionsList = [],
  ...
  void placeIdToGeocode(String placeId) async {
    ref
        .read(disposalProvider.notifier)
        .placeToGeocode(DisposalPlaceToGeocodeEntities(
  ...
  void getLatLong(List<DisposalGeocodeList> geocode) {
    DisposalGeometry val = geocode[0].geometry!;

    setState(() => {
          ref.read(disposalNotifierProvider).setLocationGeometry(
  ...
  void setPickupAddress(String query) async {
    if (widget.label == 'Pickup Location') {
      ref.read(disposalNotifierProvider).setLocation(query);
    }
  }

  Widget predictionsPlaceList(
      {required VoidCallback onTap, required DisposalPredictions predictions}) {
    final textTheme = context.textTheme;
    return Padding(
