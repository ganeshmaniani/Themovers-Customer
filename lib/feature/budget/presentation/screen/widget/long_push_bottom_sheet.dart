...
class _LongPushBottomSheetConsumerState
  ...
  void initialLongPushDetail() async {
    ...
    ref.read(budgetProvider.notifier).getLongPushType().then((res) => res.fold(
          (l) => {
  ...
  void longPushTypeList(List<LongpushType> type) async {
    ref.read(budgetControllerProvider).setLongPushType(type);
  }

  void longPushSelectedIndex(int index) async {
    ref.read(budgetControllerProvider).setLongPushIndex(index);
  }

  @override
  Widget build(BuildContext context) {
    final textTheme = context.textTheme;
    int selectedIndex = ref.watch(budgetControllerProvider).longPush ?? 0;
                          ...
                          children: [
                            ...
                            Text.rich(
                              TextSpan(
                                children: [
                                  TextSpan(
                                    text: "${longPushType[i].description}\n",
